...
void bmp390_setup_i2c()
{
    ...
    i2c_write_data(I2C_NUM_1, BMP390_ADDR, CONFIG_BMP390, IIR_FILT_7);

    vTaskDelay(10 / portTICK_PERIOD_MS);
    i2c_write_data(I2C_NUM_1, BMP390_ADDR, ODR_REG, ODR_50HZ);

    vTaskDelay(10 / portTICK_PERIOD_MS);
    i2c_write_data(I2C_NUM_1, BMP390_ADDR, OSR_REG, OSR_TEMP_NO_OSR | OSR_PRES_8X);

    vTaskDelay(10 / portTICK_PERIOD_MS);
    i2c_write_data(I2C_NUM_1, BMP390_ADDR, PWR_CTRL, MODE_NORMAL | TEMP_ON | PRESS_ON);
}
...
{
    float alt_baro = (44330.0f * (1.0f - powf(baro->press / baro->gnd_press, 0.19029f))) - ((baro->temp - baro->init_temp) * 0.0453f);

    baro->altitude_m = baro->altitude_m * 0.985f + alt_baro * 0.015f;
    float diff = alt_baro - baro->altitude_m;

    if (diff > 0.4f) diff = 0.4f;
    else if (diff < -0.4f) diff = -0.4f;

    if (diff >= 0.2f || diff <= -0.2f) baro->altitude_m += diff * 0.12f;
